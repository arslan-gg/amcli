...
export function mount(main) {
  ...
  page.append(toolbar({ title: "Statistics", titleIcon: "stats", meta: d.model.name }), body);
  ...
  const layerRows = LAYERS.filter((l) => byLayer.has(l)).map((l) => ({
    label: l, value: byLayer.get(l), swatch: layerFill(l), href: href("elements", null, { folder: "" }),
  }));
  const typeRows = [...byType.entries()].sort((a, b) => b[1] - a[1]).map(([t, n]) => ({
    label: t, value: n, swatch: typeOf(t).fill, href: href("elements"),
  }));
  const kindRows = [...byKind.entries()].sort((a, b) => b[1] - a[1]).map(([t, n]) => ({
    label: relLabel(t), value: n, href: href("relations"),
  }));
